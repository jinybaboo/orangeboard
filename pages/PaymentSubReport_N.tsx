...
import { OrangeBtnPress, OrangeBtnTxt, Space} from "../common/commonStyledComp";
...
import { ActivityIndicator, Alert, Keyboard, Platform, Pressable } from "react-native";
...
import { getPayment_SubscriptionProduct, insertReportPaymentAndroid, insertReportPaymentIos } from "../common/fetchData";
...
export const PaymentSubReport_N = (props:any) => {
    const dispatch = useAppDispatch();
    const navigation = useNavigation();

    let {ProductOptionId, price, appPrice, productName, appProductName, appProductNameIOS, appProductPlanId, isTrialAvailable, periodType}:any =  props.route.params.param.data;
        ...
        setTimeout(()=>{
            purchaseUpdateSubscription = purchaseUpdatedListener(
                async (purchase: ProductPurchase | SubscriptionPurchase) => {
                    ...
                    if (receipt) {
                        try {
                            const response:any = resultReqCount ===0 && await validateReceiptOnServer(receipt);
    ...
    const reportPriceStr = thousandComma(appPrice);
    ...
    return (
        ...
        <ScrollView>
            ...
            <SubInfoBox>
                ...
                <SubInfoTxt>{subData?.subscriptionInfo?.startDd} ~ {subData?.subscriptionInfo?.endDd}</SubInfoTxt>
            </SubInfoBox>
            <SubInfoBox>
                <SubInfoTxt>구독 가격</SubInfoTxt>
                <SubInfoTxt>{thousandComma(subData?.subscriptionInfo?.schedulePrice)}원/{periodType==='year'?'년':'월'}</SubInfoTxt>
            </SubInfoBox>
            <SubInfoBox>
                <SubInfoTxt>다음 결제일</SubInfoTxt>
                <SubInfoTxt>{subData?.subscriptionInfo?.scheduleDd}</SubInfoTxt>
            </SubInfoBox>

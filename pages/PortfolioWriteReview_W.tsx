...
const PortfolioWriteReview_W = (props:any) => {
    const [isLoading, setIsLoading] = useState(true);
    const navigation:any = useNavigation();
    const webViewRef:any = useRef(null);
    const webviewUrl = `${BASE_URL}/portfolio/writeReview?isApp=app`;
    ...
    // 이 페이지에서 돌아가면 데이터 및 피드 리프레시 되도록 설정
    useEffect(() => {
        return () => {
            DeviceEventEmitter.emit('backFromWriteReviewReload');
        }
    }, []);
...
export default PortfolioWriteReview_W;
...
const StudyContentWrite_W = (props:any) => {
    const {studyPath} = props.route.params.param;
    
    const [isLoading, setIsLoading] = useState(true);
    const navigation:any = useNavigation();
    const webViewRef:any = useRef(null);
    const webviewUrl = `${BASE_URL}/study/contentWrite?isApp=app&studyPath=${studyPath}`;
    ...
    // 이 페이지에서 스터디 복귀시 리프레시
    useEffect(() => {
        return () => {
            DeviceEventEmitter.emit('backFromStudyContentWrite');
        }
    }, []);
...
export default StudyContentWrite_W;